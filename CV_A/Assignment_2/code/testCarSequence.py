...
def updatefig(i, plot_idx = [0,99,199,299,399]):
    ...
    im.set_array(seq[:,:,i])
    if i in plot_idx:
        plt.savefig("result/tracking_1_3_cars_" + str(i) + ".png", bbox_inches='tight', pad_inches=0)
    return im,
...
##### Run LK Tracking
rect = np.array([59.0, 116.0, 145.0, 151.0], dtype=np.float32).T
...
for i in tqdm(range(1, seq.shape[2])):
    ...
    It = seq[:,:,i-1]
    It1 = seq[:,:,i]
    p = LucasKanade(It, It1, rect, threshold, num_iters, np.array([0.0,0.0]))
...
for i in range(5):
    axarr[i].imshow(plt.imread(f"result/tracking_1_3_cars_" + str(plot_idx[i]) + ".png"))
