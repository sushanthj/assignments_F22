...
class Vehicle:
    ...
    def update(self, acc, delta):
        ...
        # refer to image in same folder to see how x,y and yaw are updated based on the yaw (psi) angles we find
        self.vel += acc
        self.yaw += ((self.vel*math.tan(delta))/(WB))*dt # as image gives us psi_dot and here we calc every timestep therefore we can use dt
...
class Controller:
    ...
    def PurePursuitcontrol(self, error):
        #TODO- find delta
        delta = math.atan2((2*LENGTH*math.sin(error)),L)
...
def main():
    ...
    while getDistance([ego.x, ego.y], goal) > 1:
        ...
        # The yaw error is a combination of the current heading of the vehicle and the slope of the desired lookahead point
        slope_of_lookahead = math.atan((target_point[1]-ego.y)/[target_point[0]-ego.x])
        yaw_err =  slope_of_lookahead - ego.yaw  #TODO- Update the equation
