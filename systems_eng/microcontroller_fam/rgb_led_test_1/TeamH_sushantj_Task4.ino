...
unsigned long elementprevDebounceTime = 0;
unsigned long elementdebounceDelay = 50;
...
void loop() {
  ...
  if ((millis() - prevDebounceTime) > debDelay) {
  ...
  if (element_reading != elementlastButtonState) {
    // reset the debouncing timer
    elementprevDebounceTime = millis();
  }

  if ((millis() - elementprevDebounceTime) > elementdebounceDelay) {
    // whatever the reading is at, it's been there for longer than the debounce
    // delay, so take it as the actual current state:
  ...
  if (phase_track == 1) {
    ...
    if (intensity < 256 && intensity > -1 && color == 'r') {
      ...
      Serial.println(" ");
      intensity_2 = 255 - intensity;      
      analogWrite(red_light_pin, intensity_2);
      }
    else if (intensity < 256 && intensity > -1 && color == 'g') {
      Serial << "given color is " << color << " and given intensity is " << intensity;
      Serial.println(" ");
      intensity_2 = 255 - intensity; 
      analogWrite(green_light_pin, intensity_2);
      }
    else if (intensity < 256 && intensity > -1 && color == 'b') {
      Serial << "given color is " << color << " and given intensity is " << intensity;
      Serial.println(" ");
      intensity_2 = 255 - intensity; 
      analogWrite(blue_light_pin, intensity_2);
      }
    else {
